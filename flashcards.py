...
def run_flashcard_mode(questions, day):
    ...
    # ─── If we've exhausted all questions for this round ────────────────
    if st.session_state.flashcard_index >= total:
        # Compute accuracy
        accuracy = 0.0
        if total > 0:
            accuracy = (st.session_state.correct_count / total) * 100.0

        st.success("🎉 You've completed all questions for this round.")
        st.info(f"Your score: {st.session_state.correct_count} / {total}  "
                f"({accuracy:.1f}%)")
        ...
        # Offer a button to start an entirely new round of the same 40 questions
        if st.button("🔄 Start Another Round"):
            ...
            # 2) Reset session state: new random order, reset index & correct_count
            st.session_state.flashcard_order = list(range(total))
            random.shuffle(st.session_state.flashcard_order)
            st.session_state.flashcard_index = 0
            st.session_state.flashcard_submitted = False
            ...
            # 3) Overwrite flashcard_state on disk with the brand‐new values
            ...
            # Return immediately so Streamlit re-runs with the new state
            return

        return  # nothing else to show past the "round completed" screen
    ...
    if st.button("Submit"):
        if not selected_keys:
            ...
            if chosen == correct:
                ...
                st.error("❌ Incorrect.")
                st.markdown(f"**Correct answer(s):** {', '.join(sorted(correct))}")

                # Log a mistake to mistakes.json
    ...
    if st.button("Next"):
        if not st.session_state.flashcard_submitted:
            ...
            # Clear out the per‐option checkboxes so they don't stay checked
            for k in q["options"].keys():
                st.session_state.pop(f"opt_{k}", None)

            # Advance index and reset "submitted" flag
            st.session_state.flashcard_index += 1
            st.session_state.flashcard_submitted = False

            # Save updated index & correct_count to disk
            flashcard_state[today_key] = {
                "order": st.session_state.flashcard_order,
                "index": st.session_state.flashcard_index,
                "correct_count": st.session_state.correct_count
            ...
            # Return now so Streamlit re-runs with the new index
            return
